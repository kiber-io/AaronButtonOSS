...
Aaron Button — аксессуар для чехлов PITAKA. Официальное [приложение PITAKA](https://play.google.com/store/apps/details?id=com.pitaka.shortcuts) доступно в Google Play.
...
В OSS-клиенте добавлены три действия, которых нет в оригинальном приложении:

| Действие | Описание |
| --- | --- |
| Run Termux command | Выполняет команду в Termux в фоновом режиме. |
| Custom intent | Выполняет вручную настроенный Android Intent. |
| Custom value | Записывает произвольное значение действия в исходный NFC payload. |

`Custom value` предназначен для экспериментов с неописанными или неподдерживаемыми
значениями действий. Значение записывается как есть; при нажатии Aaron Button OSS
сможет выполнить его только если оно соответствует действию, известному приложению.
